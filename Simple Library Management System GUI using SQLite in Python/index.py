    ...
    def __init__(self, master):
        ...
        def showsummary(self):
            ...
            book_instock_counter = cur.execute(
                "SELECT COUNT(book_id) FROM books WHERE book_status"
            ).fetchall()
        ...
        self.btn_add_member = Button(
            top_frame, text="Add New Member", font="arial 12 bold", padx=10, command=self.new_member
        )
        self.btn_add_member.pack(side=LEFT)

        self.btn_add_book = Button(
            top_frame, text="Add New Book", font="arial 12 bold", padx=10, command=self.new_book
        )
        self.btn_add_book.pack(side=LEFT)

        self.btn_issue_book = Button(
            top_frame, text="Issue Book", font="arial 12 bold", padx=10, command=self.issue_book
        )
...
def base():
    ...
    base_window.title("MASA LibraVault Manager")
